...
pub async fn unban_command(ctx: Context, command: ApplicationCommandInteraction) -> Result<(), SerenityError> {
    ...
    input_slashcommand(&("unban ".to_owned() + &v1arg.to_string()));
    _ = command.create_interaction_response(&ctx.http, |response| {
        response.kind(InteractionResponseType::ChannelMessageWithSource)
            .interaction_response_data(|data| {
                data.ephemeral(false)
                    .embed(|embed| {
                        embed.title("Issued Admin Command")
                            ...
                            .fields(vec![
                                ("Command Executed:", "/unban", true),
                                ("Arguements: ", &v1arg.to_string(), true),
