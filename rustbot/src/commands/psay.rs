...
pub async fn psay_command(ctx: Context, command: ApplicationCommandInteraction) -> Result<(), SerenityError> {
    ...
    if let CommandDataOptionValue::Integer(str) = command.data.options.get(0).expect("msg").resolved.as_ref().expect("msg") {
        v1arg = str.to_string();
    }

    if let CommandDataOptionValue::String(str) = command.data.options.get(1).expect("msg").resolved.as_ref().expect("msg") {
        v2arg = str.to_string();
    }
    
    let append_cmd = format!("{} {}", v1arg, v2arg);
    input_slashcommand(&("psay ".to_owned() + &append_cmd.to_string()));
    _ = command.create_interaction_response(&ctx.http, |response| {
        response.kind(InteractionResponseType::ChannelMessageWithSource)
            .interaction_response_data(|data| {
